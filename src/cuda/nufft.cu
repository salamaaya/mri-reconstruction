...
__global__ void nufft_forward(const Complex *, int, int,
                   const float *, const float *, int, Complex *);
__global__ void nufft_adjoint(const Complex *, int,
           ...
           Complex *samples, Complex *out)
{
    ...
    nufft_forward<<<gridSize, BLOCK_SIZE>>>(img_d, nx, ny, kx_d, ky_d, m, samples_d);
    cudaDeviceSynchronize();
    nufft_adjoint<<<gridDim, blockDim>>>(samples_d, m, kx_d, ky_d, nx, ny, out_d);
    cudaDeviceSynchronize();
